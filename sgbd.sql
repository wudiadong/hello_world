...
--refDevis: cet valeur est cree par le procedure demande_devis chez nous
--cle: cette cle est donne par l'acheteur
--etat: pas_paie ou deja_paie
--Editeur:Muhui YU
create table vente(
idv integer,
refDevis integer,
...
constraint check_etat check(etat='pas_paie' or etat='deja_paie')
);
	
/*$$$$$$$$$$$$$$$$$$$$$ Table commande $$$$$$$$$$$$$$$$$$$$$$$*/
--Cet tableau pour noter les achats que SuperMachine a fait
--cle_autorisation:cette cle est cree par notre entreprise
--refDevis: cet valeur est cree par le procedure demande_devis chez vendeur
--Editeur:Muhui YU
...
--prixHT:l'argent de notre entrepris gagne apres CCI avoir pris la taxe
--Editeur:Muhui YU
...
SiretAcheteur number,
SiretBanqueAcheteur number,
cle_autorisation varchar2,
refDevisAcheteur integer)
return integer
is
    cursor r is
	...
	fini_recherche:=0;	
        ...
        /* si l'entrepise a ce devis */
			...
			/* si c'est une commande existant dans le tableau vente. on ne crée pas un nouveau record dans le tableau vente.*/
            if r1%notfound then 
            ...
            execute immediate paiement using out resultat, in SiretAcheteur,in SiretBanqueAcheteur,in siretVendeur,in cle_autorisation,in prix_totalv,in description;
			  if resultat=1 then
			  /* si le paiement d'acheteur a bien recu.on va envoyer les marchandise a acheteur*/
				...
				login:='begin :1:= '||loginCCI||'.LoginParSiret('||SiretAcheteur||');end;';
                execute immediate login using out loginAcheteur;
			   ...
			   dbms_output.put_line('Desole!Votre paiement ne est pas reussi!**SuperMachine**');
			   return 0;
               end if;
        end if;
end;
...
--procedure acheter() utilisee pour appeler la function creerAutorisation() en Banque , la function achat() en vendeuse et recorde tout les commandes nous faisons.
--Editeur:Muhui YU
...
is
  ...
  achat varchar2(255);
  resultat integer;
  cle_autorisation varchar2(255);
  cle varchar2(255);
begin
  select loginOracle into loginBanque from information where nom='banque';
  cle:='begin :1:='||loginBanque||'.creerAutorisation('||siretVendeur||');end;';
  execute immediate cle using out cle_autorisation;
  dbms_output.put_line('cle_autorisation est: '||cle_autorisation);
  ...
  insert into commande values (seq_idc.nextval,loginVendeur,refArt,cle_autorisation,refDevis);
...
--execute ngiovin_a.consultationCompte(7);
...
--function:Pour prendre le siret de l'entreprise par faire inscription on CCI.
...
begin
  ...
  insert into information values (seq_idin.nextval,'banque','ngiovin_a','10000000000002',compteBanque);
end;
/
*/
